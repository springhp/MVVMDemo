...
import com.hp.jetpack.demo.databinding.FragmentSmartRefreshLayoutBinding
...
class SmartRefreshLayoutFragment : BaseFragment<RoomViewModel, FragmentSmartRefreshLayoutBinding>() {
    override fun layoutId(): Int = R.layout.fragment_smart_refresh_layout
    var mData = mutableListOf<User>()
    lateinit var adapter: RoomAdapter
    override fun initView(savedInstanceState: Bundle?) {
        mDataBinding.toolbar.initClose(titleStr = "SmartRefreshLayout", backImg = R.drawable.ic_back) {
            nav().navigateUp()
        }

        adapter = RoomAdapter(mData)
        mDataBinding.rlv.adapter = adapter
        mDataBinding.rlv.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
